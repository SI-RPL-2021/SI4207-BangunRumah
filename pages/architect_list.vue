<template>
  <div>
    <v-img :src="require(`~/assets/img/headerfindarchitect.png`)">
      <v-container>
    ...
    <v-container>
      <v-row>
        ...
        <v-col xl="8" lg="8" md="8" sm="12" xs="12">
          <v-row>
            <v-col
              v-for="architect in dataArchitect"
              :key="architect.id"
              xl="4"
              lg="4"
              md="4"
              ...
              <v-card max-width="344">
                ...
                <v-card-actions>
                  <v-btn
                    ...
                    dark
                    block
                    @click="selectedArchitect(architect)"
                  >
                    Contact
                  </v-btn>
                </v-card-actions>
    ...
    <v-dialog v-model="showDetailDataArchitect" width="500">
      <v-card>
        <v-card-title class="headline">
          ...
          <v-btn
            icon
            color="red"
            @click.native="showDetailDataArchitect = false"
          >
...
export default {
  ...
  mounted() {
    this.getDataArchitect()
  },
  methods: {
    ...mapActions({
      showHideSnackbar: 'snackbars/showHideProcess',
    }),
    async getDataArchitect() {
      const { data } = await this.$axios.get('architect/data')
      this.dataArchitect = data.dataArchitect
    },
    selectedArchitect(data) {
      this.idWorker = data.id
      this.name = data.name
      this.email = data.email
      this.address = data.address
      this.phone = data.phone
      this.showDetailDataArchitect = true
    },
    async selectedAddress() {
      const { data } = await this.$axios.post('architect/data/filter/city', {
    ...
    openDialogOrder() {
      ...
      this.showDetailDataArchitect = false
    },
    async createOrder() {
