<template>
  <div>
    <v-img :src="require(`~/assets/img/headerfindworker.png`)">
      <v-container>
    ...
    <v-container>
      <v-row>
        ...
        <v-col xl="8" lg="8" md="8" sm="12" xs="12">
          <v-row>
            <v-col
              v-for="designer in dataDesigner"
              :key="designer.id"
              xl="4"
              lg="4"
              md="4"
            ...
            >
              <v-card max-width="344">
                <v-img
                  :src="require(`~/assets/img/finddesigner.png`)"
                  height="200px"
                />
                ...
                <v-card-actions>
                  <v-btn
                    ...
                    dark
                    block
                    @click="selectedDesigner(designer)"
                  >
                    Contact
                  </v-btn>
                </v-card-actions>
    ...
    <v-dialog v-model="showDetailDataDesigner" width="500">
      <v-card>
        <v-card-title class="headline">
          ...
          <v-btn
            icon
            color="red"
            @click.native="showDetailDataDesigner = false"
          >
...
export default {
  ...
  mounted() {
    this.getDataDesigner()
  },
  methods: {
    ...mapActions({
      showHideSnackbar: 'snackbars/showHideProcess',
    }),
    async getDataDesigner() {
      const { data } = await this.$axios.get('designer/data')
      this.dataDesigner = data.dataDesigner
    },
    selectedDesigner(data) {
      this.idWorker = data.id
      this.name = data.name
      this.email = data.email
      this.address = data.address
      this.phone = data.phone
      this.showDetailDataDesigner = true
    },
    async selectedAddress() {
      const { data } = await this.$axios.post('designer/data/filter/city', {
    ...
    openDialogOrder() {
      ...
      this.showDetailDataDesigner = false
    },
    async createOrder() {
